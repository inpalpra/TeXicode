...
# Add src to sys.path so we can import TeXicode modules
sys.path.append(os.path.abspath('src'))

import renderer
import pipeline

# 1. Define the environments and their delimiters (left, right)
MULTI_LINE_ENVS = {
    ('m','a','t','r','i','x'):          ('', ''),
    ('p','m','a','t','r','i','x'):      ('(', ')'),
    ('b','m','a','t','r','i','x'):      ('[', ']'),
...
# 3. Patch render_matrix to support varying column alignments (l, c, r)
def patched_render_matrix(row_sketches, delim_left, delim_right, align_spec=None):
    ...
    if delim_left and delim_right:
        left, _, _ = renderer.util_delimiter(delim_left, grid_height, grid_horizon)
        right, _, _ = renderer.util_delimiter(delim_right, grid_height, grid_horizon)
        result = []
        for r in range(grid_height):
            result.append(left[0][r] + [renderer.arts.bg] + grid[r]
                          + [renderer.arts.bg] + right[0][r])
...
# 4. Patch _render_opn_root to apply environments grouping logically
